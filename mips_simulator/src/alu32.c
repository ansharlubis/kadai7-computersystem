...
void alu32_init(ALU32 *alu32, Path *ops, Bus *a, Bus *b, Bus *s){
	...
	Path *null, *set, *carryout;
	null = (Path *)malloc(sizeof(Path));
	set = (Path *)malloc(sizeof(Path));
	carryout = (Path *)malloc(sizeof(Path));

	path_init(null); path_init(set); path_init(carryout);
	...
	alu_msb_init(alu32 -> alumsb, ops, ops+1, ops+2, a -> p+31, b -> p+31, null, c+30, s -> p+31, carryout, set);
...
void alu32_driver(int na, int nb, Signal o1, Signal o2, Signal bin){
	...
	alu32_run(&alu32);
	printf("ALU32(op: %d, %d)(bin: %d)(a: %d, b: %d) => %d\n", path_get_signal(&ops+1), path_get_signal(&ops), path_get_signal(&ops+2) , na, nb, bus_get_value(&s));
	alu32_release(&alu32);
